...
# Iterate arguments
seq_len_list=("10" "50" "100")
partition_list=("a100") # "a100" "feita100" "deeplearn"
...
model_list=("TimesNet" "COUTA" "DeepSVDDTS"  "TranAD") # "TimesNet" "COUTA" "DeepSVDDTS"  "TranAD"
sigma_list=("0.1" "0.5" "1.0") # "0.1" "0.5" "1.0" "2.0" "5.0"
window_size_list=("2" "5" "10")
                ...
                for seq_len in "${seq_len_list[@]}"; do
                    for sigma in "${sigma_list[@]}"; do
                        for window_size in "${window_size_list[@]}"; do
                            ...
                            arguments="--save_model $save_model --data_train $data_train --data_test $data_test --data_test_label $data_test_label --exps_root $exps_root --exp_name $exp_name --subset_size $subset_size --model $model --seed $seed --seq_len $seq_len --stride $stride --epochs $epochs --epoch_steps $epoch_steps --batch_size $batch_size --lr $lr --sigma $sigma --window_size $window_size --smooth_count $smooth_count"
